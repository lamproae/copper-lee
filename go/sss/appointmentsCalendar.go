...
func dismissData(st1 int, st2 bool) {
	//do nothings in particular for now
}
...
func schedule(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := Users[name]; ok {
		if Users[name].Times[intTimeVal] == true {
			...
			mutex.Unlock()
			fmt.Println("Users exists, variable should be modified")
			t, _ := template.ParseFiles("generic.txt")
			page := &Page{Title: "Successfull Scheduled!", Body: template.HTML("This appoinment has been scheduled. <a href='/users'> Back to users</a>")}
			t.Execute(w, page)
		} else {
...
func main() {
	...
	r.HandleFunc("/scheduled/{name:[A-Za-z]+}/{hour:[0-9]+}", schedule)
	http.Handle("/", r)

	err := http.ListenAndServe("10.71.1.60:1900", nil)
	if err != nil {
