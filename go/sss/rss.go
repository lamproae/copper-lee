...
func grabFeed(feed *Feed, feedChan chan bool, osvg *svg.SVG) {
	...
	fmt.Println("Grabbgin feed ", feed.url, "at", startGrabSeconds, " second mark")
	...
	if feed.status == 0 {
		...
		rssFeed := rss.New(timeout, true, channelHandler, itemHandler)

		if err := rssFeed.Fetch(feed.url, nil); err != nil {
			fmt.Println(os.Stderr, "[e] %s: %s", feed.url, err)
			return
		} else {
		...
		fmt.Println("Fedd already in progress")
	}
}
...
func main() {
	...
	err := http.ListenAndServe("10.71.1.60:2900", nil)
	if err != nil {
